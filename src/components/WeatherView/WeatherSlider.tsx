...
const WeatherSlider: React.FC = () => {
    ...
    if (loading) {
        return (
            <div className="flex flex-col justify-between flex-1 mt-6 text-white animate-pulse">
                {/* Main Temperature Hero Skeleton */}
                ...
                {/* 7-Day Forecast Carousel Skeleton */}
                <div className="bg-black/30 backdrop-blur-md rounded-2xl p-3 border border-white/15 shadow-xl shrink-0 mt-auto">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="bg-white/15 backdrop-blur-sm p-2.5 rounded-xl border border-white/15 flex flex-col items-center">
    ...
    return (
        <section aria-label="Current temperature and 7-day daily forecast" className='flex flex-col justify-between flex-1 mt-6 text-white'>
                ...
                <div className="flex items-baseline justify-between mb-2">
                    <div>
                        <h2 className='text-6xl md:text-7xl font-extrabold tracking-tight text-white drop-shadow-lg leading-none'>
                            {current?.temp ?? '--'}°<span className="text-3xl font-normal text-white/80">{unit}</span>
                        </h2>
                    ...
                    <div className="text-right bg-black/20 backdrop-blur-md px-3 py-2 rounded-2xl border border-white/15">
                        ...
                        <p className="text-xl font-extrabold text-white">{current?.feelsLike}°{unit}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-black/30 backdrop-blur-md rounded-2xl p-3 border border-white/15 shadow-xl shrink-0 mt-auto">
                <div className="flex justify-between items-center mb-2 px-1">
                    <h3 className="text-[11px] font-bold text-white/90 uppercase tracking-wider">
                    ...
                    {daily.map((day, index) => (
                        <div key={day.date || index} className="px-1 text-center select-none">
                            <div className="bg-white/15 hover:bg-white/25 transition-all p-2.5 rounded-xl backdrop-blur-sm border border-white/15 flex flex-col items-center">
                                ...
                                <p className="text-[10px] font-medium text-white/90 truncate w-full">{day.condition?.label}</p>
