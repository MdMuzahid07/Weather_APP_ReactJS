...
const SearchModal: React.FC = () => {
  const { isSearchOpen, setIsSearchOpen, selectLocation, searchCities, fetchUserLocation } = useWeatherStore();
  ...
  const popularCities: LocationItem[] = [
      ...
      <div className="glass-panel rounded-3xl p-6 w-full max-w-lg shadow-2xl transition-all border border-slate-800 text-slate-100">
        <div className="flex justify-between items-center mb-4">
          <h2 id="modal-title" className="text-lg font-bold text-slate-100 flex items-center gap-2">
            <MapPin size={20} className="text-sky-400" aria-hidden="true" /> Select Location
          </h2>
          <button
            onClick={() => setIsSearchOpen(false)}
            aria-label="Close location search modal"
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-800 text-slate-400 hover:text-white transition-colors focus-visible:ring-2 focus-visible:ring-sky-400"
          >
        ...
        <div className="relative mb-4">
          ...
          {searching && (
            <span className="absolute right-3.5 top-3 text-xs text-sky-400 font-semibold animate-pulse" aria-live="polite">
              Searching...
            </span>
          )}
        </div>
        ...
        <button
          ...
          className="w-full mb-4 py-2.5 px-4 rounded-xl bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 border border-sky-500/30 font-bold flex items-center justify-center gap-2 transition-all group text-sm cursor-pointer focus-visible:ring-2 focus-visible:ring-sky-400"
        >
          <Navigation size={18} className="group-hover:rotate-45 transition-transform" aria-hidden="true" />
          ...
          <div className="max-h-60 overflow-y-auto space-y-1.5 pr-1">
            {results.map((city, idx) => (
              <button
                ...
                className="w-full text-left p-3 rounded-xl bg-slate-800/40 hover:bg-slate-800 flex justify-between items-center transition-colors border border-slate-800 hover:border-sky-500/30 cursor-pointer focus-visible:ring-2 focus-visible:ring-sky-400"
              >
                <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {popularCities.map((city) => (
                <button
                  key={city.name}
                  onClick={() => selectLocation(city)}
                  className="text-left p-3 rounded-xl bg-slate-800/40 hover:bg-slate-800 text-slate-200 hover:text-sky-400 font-medium text-xs transition-all border border-slate-800 cursor-pointer focus-visible:ring-2 focus-visible:ring-sky-400"
                >
