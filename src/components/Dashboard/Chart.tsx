...
interface CustomTooltipProps {
    ...
    label?: string;
    unit: string;
    activeTab: 'temp' | 'rain' | 'uv';
    theme: string;
}

const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label, unit, activeTab, theme }) => {
...
const Chart: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'temp' | 'rain' | 'uv'>('temp');
    ...
    return (
        <section aria-label="24-Hour Forecast Breakdown" className={`rounded-2xl p-4 mb-4 shrink-0 border transition-all ${
            theme === 'dark'
            ...
            <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
                ...
                {/* Tab Switcher */}
                <div role="tablist" aria-label="Chart metrics" className={`flex items-center gap-1 p-1 rounded-xl border ${
                    ...
                    <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-[11px] font-bold transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-sky-400 ${
                            activeTab === 'temp'
