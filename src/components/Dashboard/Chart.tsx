...
import { TemperatureUnit, ThemeMode } from '../../types/weather';

type ChartTab = 'temp' | 'rain' | 'uv';
...
const CustomTooltip = ({ active, payload, label, unit, activeTab, theme }: CustomTooltipProps) => {
  if (active && payload && payload.length) {
    return (
      <div
        className={`p-3 rounded-xl shadow-2xl border text-xs font-semibold space-y-1 ${
          theme === 'dark'
      ...
      >
        <p
          className={`font-bold border-b pb-1 ${theme === 'dark' ? 'text-slate-400 border-slate-800' : 'text-slate-500 border-slate-100'}`}
        >
          {label}
        </p>
        {activeTab === 'temp' && (
...
const Chart = () => {
      ...
      >
        ...
        <div className="h-[140px] w-full bg-slate-800/20 dark:bg-slate-800/40 rounded-xl flex items-end p-2 gap-2 border border-slate-800/30">
          {[...Array(12)].map((_, i) => (
            <div
              key={i}
              className="flex-1 bg-slate-700/30 dark:bg-slate-700/40 rounded-t"
  ...
  return (
    <div
      className={`rounded-2xl p-4 mb-4 shrink-0 border transition-all ${
        theme === 'dark'
    ...
    >
      ...
      <div className="h-[140px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={hourlyData} margin={{ top: 5, right: 5, bottom: -10, left: -25 }}>
            <defs>
