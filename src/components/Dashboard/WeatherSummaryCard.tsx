...
import { Shirt, Umbrella, Activity, SunMedium, Compass } from 'lucide-react';

const WeatherSummaryCard: React.FC = () => {
  ...
  return (
    <section aria-label="Smart Daily Insights" className={`rounded-2xl p-4 mb-4 shrink-0 transition-all border ${
      theme === 'dark'
      ...
      <div className="flex justify-between items-center mb-3">
        <div className="flex items-center gap-2">
          <Compass size={17} className="text-sky-400 motion-safe:animate-spin-slow" aria-hidden="true" />
          <h2 className={`text-xs font-bold tracking-tight ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {/* 1. What to Wear */}
        <article className={`p-3 rounded-xl border flex items-start gap-3 transition-colors ${
          theme === 'dark'
            ? 'bg-slate-800/40 border-slate-700/50 hover:border-amber-500/30'
            : 'bg-amber-50/70 border-amber-200/80 hover:bg-amber-50'
        }`}>
          <div className="w-9 h-9 rounded-xl bg-amber-500/15 text-amber-600 dark:text-amber-400 flex items-center justify-center shrink-0 mt-0.5" aria-hidden="true">
            <Shirt size={18} />
          </div>
          <div className="min-w-0">
            <div className="flex items-center justify-between gap-1">
              <h3 className={`text-xs font-bold truncate ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`}>
            ...
            <p className={`text-[11px] font-medium mt-1 leading-tight line-clamp-2 ${theme === 'dark' ? 'text-slate-400' : 'text-slate-600'}`}>
              {insights.clothing.text}
            </p>
          </div>
        </article>

        {/* 2. Outdoor Activity Index */}
        <article className={`p-3 rounded-xl border flex items-start gap-3 transition-colors ${
          theme === 'dark'
            ? 'bg-slate-800/40 border-slate-700/50 hover:border-sky-500/30'
            : 'bg-blue-50/70 border-blue-200/80 hover:bg-blue-50'
        }`}>
          <div className="w-9 h-9 rounded-xl bg-sky-500/15 text-sky-600 dark:text-sky-400 flex items-center justify-center shrink-0 mt-0.5" aria-hidden="true">
            <Activity size={18} />
          </div>
          <div className="min-w-0">
            <div className="flex items-center justify-between gap-1">
        ...
        <article className={`p-3 rounded-xl border flex items-start gap-3 transition-colors ${
          ...
          <div className="w-9 h-9 rounded-xl bg-teal-500/15 text-teal-600 dark:text-teal-400 flex items-center justify-center shrink-0 mt-0.5" aria-hidden="true">
