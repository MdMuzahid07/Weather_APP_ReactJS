...
const WeatherSlider = lazy(() => import('../components/WeatherView/WeatherSlider'));
        ...
        <>
            ...
            <div className={`w-full max-w-6xl h-full md:h-[94vh] max-h-[920px] mx-auto flex flex-col md:flex-row glass-panel rounded-none md:rounded-3xl border-0 md:border shadow-2xl overflow-hidden transition-all duration-300 ${
                ...
                >
                    <WeatherViewHeader />
                    <WeatherLocation />
                    <ErrorBoundary fallbackTitle="Hero Forecast Unavailable">
                        <Suspense fallback={<div className="h-48 w-full bg-white/10 rounded-2xl animate-pulse"></div>}>
                            <WeatherSlider />
                        </Suspense>
                    </ErrorBoundary>
                </div>
                ...
                <div className={`w-full md:w-7/12 lg:w-8/12 p-4 md:p-5 flex flex-col justify-between overflow-y-auto md:overflow-hidden flex-1 no-scrollbar transition-colors ${
                    ...
                    <Header />
                    <ErrorBoundary fallbackTitle="Smart Insights Unavailable">
                        <WeatherSummaryCard />
                    </ErrorBoundary>
                    <ErrorBoundary fallbackTitle="24-Hour Chart Unavailable">
                        <Suspense fallback={<div className="h-36 w-full bg-slate-800/40 rounded-2xl animate-pulse"></div>}>
                            <Chart />
                        </Suspense>
                    </ErrorBoundary>
                    <ErrorBoundary fallbackTitle="Weather Metrics Unavailable">
                        <TodayWeatherDetails />
                    </ErrorBoundary>
                </div>
            </div>
